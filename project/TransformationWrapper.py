...
class TransformationWrapper(BaseEstimator, TransformerMixin):
    ...
    :param BaseEstimator: iiii
    :type BaseEstimator: list
    :param TransformerMixin: oooo
    :type TransformerMixin: list
    :return: ooooooo
    :rtype: list
    """
    ...
    def transform(self, X, y=None):
        ...
        if self.data_ != None:
            return pd.DataFrame(X.apply(
                lambda row: pd.Series(self.transformation(
                    row[self.column_name_], self.data_)),
                axis=1
            ))
        else:
            return pd.DataFrame(X.apply(
                lambda row: pd.Series(
                    self.transformation(row[self.column_name_])),
                axis=1
            ))
